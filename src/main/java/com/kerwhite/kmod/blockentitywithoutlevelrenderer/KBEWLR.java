...
public class KBEWLR extends BlockEntityWithoutLevelRenderer
{
    ...
    private void renderModelLists(ItemRenderer itemRenderer, BakedModel model, ItemStack stack,PoseStack poseStack, MultiBufferSource bufferSource,int combinedLight, int combinedOverlay) {
        ...
        for (BakedModel bakedModel : model.getRenderPasses(stack, true)) {
            for (RenderType renderType : bakedModel.getRenderTypes(stack, true)) {
                ...
                if (stack.hasFoil())
                {
                    vertexConsumer = itemRenderer.getFoilBuffer(bufferSource, renderType, true, true);
                }
                else
                {
                    vertexConsumer = itemRenderer.getFoilBuffer(bufferSource, renderType, true, false);
                }
                itemRenderer.renderModelLists(bakedModel, stack, combinedLight, combinedOverlay, poseStack, vertexConsumer);
            }
        }
    }
    @Override
